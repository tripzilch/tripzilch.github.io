(code=({copyright:piterpasma,max,floor:K,abs,sin,cos,min}=Math)=>{
  ...
  //
  O={lw:.4,d:.8,h:420,bg:1}; // defaults
  ...
  L=(x,y)=>(x*x+y*y)**.5; // vec2 length (from: Elements, Euclid 300 BC)
  A=([x,y,z],[a,b,c],t=1)=>[x+a*t,y+b*t,z+c*t]; // vec3 add
  m=0;N=([x,y,z])=>[x/(m=1e-99+(x*x+y*y+z*z)**.5),y/m,z/m]; // vec3 normalize + length calc
  ...
  G=(x,y,t)=>A(cp,N(A(A(fw,rt,Y*x),up,y)),t); // 2D+depth to 3D 
  ...
  u=([x,y])=>{ // raytrace function
    if(abs(x)<Y2-.015&&abs(y)<.5-.015){
      d=IX(cp,rd=N(A(A(fw,rt,x),up,y)),240);
      if(d<240){
        n=nl(p=A(cp,rd,d));
        for(a=0,b=.31*(IX(A(p,n,.02),lv=N(A(lp,p,-1)),ld=240<m?240:m,.02)>=ld||.6),i=1;i<6;i++,b*=.7)
          a+=max(0,b*P(A(p,n,i/2)));
        d=max(0,  max(0,D(n,lv))*a)*min(1,2.4-d*.01);
        // d=max(0,1-max(0,D(n,lv))*a)*min(1,2.4-d*.01);
        d=DR/(d+1e-3);
        return[N([D(sx,hd=N(X(n,hv))),D(sy,hd),0]),d<.3&&!Qh(QT,x-d,y-d,d*2,(u,v)=>(x-u)**2+(y-v)**2<d*d)]
      }
    }
    return[]
  };
  ...
  v=[`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${H*Y|0} ${H}" width="${M*Y}mm" height="${M}mm">\n<!-`+`- `+Date(),new U(O),,`fxhash='${fxhash}';(code=${code})()\n-`+`->`,O.bg?`<rect x="${-H}" y="${-H}" width="${H*3}" height="${H*3}" fill="#000000"/>`:``]; // init SVG
  
  function*E(){
    // flow field tracing rendering SVG creating function
    v.push([`<g fill="none" stroke="#ffffff" stroke-width="${(LW*H).toFixed(4)}" stroke-linecap="round">`]);
    hv=N(A(A(rt,fw,1/Z),up,R(.3+R(3)))); // hatch direction
    QT=Q(-1,-1,2);
    for(I=0;I<1e3;I++){ // 1e3 = 1000 fails until finished
      [f0,h]=u(q0=[R(Y)-Y2,R()-.5]); // try random point
      if(h){ // if OK then trace bidirectionally
        ...
        [m=T()<0?DR:-DR,-m].map(s=>{
          ...
          q=q0;
          pd=fq=f0;
          for(h=1;h&&D(pd,fq)>.5&&D(f0,fq)>-.7;qq.push(q))
            [fq,h]=u(q=A(q,pd=fq,s));
        });
        if(qq[9]){
          // accept only if trace has more than 10 pts
          C.beginPath(I=0); // draw trace to canvas and also add it to the SVG
          n=v.push(`<path d="M ${qq.map(([x,y])=>(Qa(QT,[x,y]),x+=Y2,y+=.5,C.lineTo(x*ch,y*ch),[x*H|0,y*H|0])).join` `}"/>`);
          C.stroke();
    ...
    // show SVG and hide canvas
    B.append(im=new Image);
    im.src=`data:image/svg+xml,`+encodeURIComponent(v.join`\n`);
    V.style.display='none'
  }

  J=_=>E.next().done||setTimeout(J); // timeout loop function
  J(E=E()) // start the render function
})() // start the program